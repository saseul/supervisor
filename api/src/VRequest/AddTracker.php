...
class AddTracker extends Request
{
    ...
    public function _GetValidity()
    {
        ...
        $verification = $verification && ($this->type === 'AddTracker');
        $verification = $verification && (mb_strlen($this->version) < 64);
        $verification = $verification && (mb_strlen($this->from) === 48);
        $verification = $verification && preg_match('/^[0-9a-z]{1,}\\.[0-9a-z]{1,}\\.[0-9a-z]{1,}(\\.[0-9a-z]{1,})?$/', $this->ip);
    ...
    {
        Tracker::SetHost($this->from, $this->ip);
